                                ...
                                <ul className={`${(settings.navCollapsed && !isOpenDrawer) && ' hidden group-hover:block'} ${isOpenDrawer && 'block'}`}>
                                    {item.subMenuItems.map((subItem, subIndex) => (
                                        <Link href={subItem.path} key={subIndex} onClick={onClose}>
                                            <li key={subIndex}
                                                className={`text-sm flex items-center gap-x-4 cursor-pointer p-2 my-2 pl-3.5 rounded-md   ${openSubMenus.includes(index) && pathName === subItem.path ? 'bg-primary text-white' : 'hover:bg-accent'}`}>
                                                <TbPointFilled/>
